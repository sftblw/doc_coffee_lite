defmodule DocCoffeeLite.Translation.ImportExport do
  @moduledoc """
  Handles Export and Import of project data using JSON metadata and ZIP archives.
  """
  ...
  @metadata_filename "doc_coffee.json"
  @source_dir "source"

  defmodule ExportManifest do
    @derive {Jason.Encoder, only: [:version, :exported_at, :project, :source_document, :translations]}
    defstruct [
      :version,
      :exported_at,
      :project,
  ...
  def export_project(project_id) do
    ...
    if is_nil(source_doc) do
      ...
      create_archive(manifest, source_doc)
    end
  end
  ...
  defp create_archive(manifest, source_doc) do
    ...
    try do
      ...
      json_content = Jason.encode!(manifest, pretty: true)
      File.write!(Path.join(tmp_dir, @metadata_filename), json_content)
  ...
  def import_to_existing(project_id, zip_path) do
    ...
    with {:ok, tmp_dir} <- unzip_archive(zip_path),
         {:ok, manifest} <- read_manifest(tmp_dir),
         :ok <- verify_compatibility(project, manifest),
         {:ok, count} <- apply_translations(project, manifest["translations"]) do
      File.rm_rf(tmp_dir)
      {:ok, count}
    else
      {:error, reason} -> 
        # Clean up if possible, though unzipping might have failed
        {:error, reason}
    end
  end
  ...
  defp verify_compatibility(project, manifest) do
    ...
    if source_doc.checksum == manifest_checksum do
      :ok
    else
  ...
  defp apply_translations(project, translations) do
    Repo.transaction(fn ->
      Enum.reduce(translations, 0, fn entry, count ->
        ...
        if unit do
          ...
          {:ok, _} = Translation.create_block_translation(%{
            translation_unit_id: unit.id,
            translation_run_id: nil, # Imported, not linked to a run
          ...
          # Mark as translated
          Translation.update_translation_unit(unit, %{status: "translated", is_dirty: entry["is_dirty"] || false})
  ...
  def import_as_new(zip_path) do
    with {:ok, tmp_dir} <- unzip_archive(zip_path),
         ...
         {:ok, _count} <- apply_translations(project, manifest["translations"]) do
      File.rm_rf(tmp_dir)
      {:ok, project}
    else
      error -> 
  ...
  defp create_project_from_manifest(manifest) do
    ...
    Translation.create_project(%{
      title: proj_data["title"] <> " (Imported)",
  ...
  defp setup_source_document(project, tmp_dir, manifest) do
    ...
    if File.exists?(extracted_source_path) do
      ...
      Translation.create_source_document(%{
        ...
        work_dir: Path.join(upload_dir, "work_#{Ecto.UUID.generate()}"), # Temp work dir for parsing
  ...
  defp parse_source_document(project, source_doc) do
    ...
    with {:ok, session} <- Epub.open(source_doc.source_path, source_doc.work_dir),
         ...
         {:ok, _run} <- RunCreator.create(project.id, status: "draft", llm_opts: [allow_missing?: true]) do
      :ok
    else
      error -> error
    end
