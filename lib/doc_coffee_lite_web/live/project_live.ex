defmodule DocCoffeeLiteWeb.ProjectLive do
              ...
              <%= if @project do %>
                <div class="grid gap-4 sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    <p :if={@project.status == "running" && @eta} class="mt-1 text-[0.65rem] text-amber-600 font-medium">
                      ETA: {@eta}
                    </p>
                  </div>
                </div>
    ...
    case Repo.get(Project, id) do
      ...
      project ->
        ...
        total = Repo.aggregate(from(u in TranslationUnit,
          ...
          where: g.project_id == ^id), :count, :id)

        completed = Repo.aggregate(from(u in TranslationUnit,
          join: g in assoc(u, :translation_group),
          where: g.project_id == ^id and u.status == "translated"), :count, :id)
  ...
  defp calculate_eta(project, completed, total) do
    with %TranslationRun{started_at: started_at} when not is_nil(started_at) <- latest_run(project),
  ...
  defp format_duration(seconds) do
    cond do
      ...
      seconds < 3600 -> "#{round(seconds / 60)}m remaining"
      true ->
